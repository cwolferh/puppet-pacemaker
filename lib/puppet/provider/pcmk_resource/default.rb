Puppet::Type.type(:pcmk_resource).provide(:default) do
  ...
  def resource_params
    ...
    get_attrs = pcs('get resource params', cmd)
  ...
  def op_params
    cmd = 'resource show ' + @resource[:name]
    get_attrs = pcs('get op params', cmd)

    # find the Attributes
    for line in get_attrs.lines.each do
      return (line.scan /Operations: (.+?)$/m)[0][0].strip if line.include? 'Operations:'
    end
    # return empty string if Attributes not found
    ''
  end
  ...
  def op_params=(value)
    cmd = 'resource update ' + @resource[:name] + ' op ' + value
    pcs('update attributes', cmd)
  end

  def meta_params
    cmd = 'resource show ' + @resource[:name]
    get_attrs = pcs('get meta params', cmd)

    # find the Attributes
    for line in get_attrs.lines.each do
      return (line.scan /Meta Attrs: (.+?)$/m)[0][0].strip if line.include? 'Meta Attrs:'
    end
    # return empty string if Attributes not found
    ''
  end
  ...
  def meta_params=(value)
    cmd = 'resource update ' + @resource[:name] + ' meta ' + value
    pcs('update attributes', cmd)
  end
